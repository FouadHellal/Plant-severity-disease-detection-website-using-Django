...
def classify_image_with_vit(image):
    # Charger le modèle ViT
    model_vit = create_vit_classifier()

    # Charger les poids du modèle ViT à partir du fichier
    model_vit.load_weights('zyraapp/vit_classifier_unet.weights.h5')

    # Prétraiter l'image
    # Assurez-vous que l'image est sous forme de tableau numpy avec les dimensions attendues par le modèle
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convertir en format RGB si nécessaire
    image = cv2.resize(image, (224, 224))  # Redimensionner l'image si nécessaire
    image = np.expand_dims(image, axis=0)  # Ajouter une dimension pour créer un lot (batch)

    # Utiliser l'image pour la classification
    predictions_vit = model_vit.predict(image)
    predicted_class_vit = np.argmax(predictions_vit, axis=1)
...
def upload_image(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Classer l'image avec ViT
            image = cv2.imread(file_path)
            severity_vit = classify_image_with_vit(image)
